# デプロイ完了 - 次のステップ

**日時**: 2025-11-30  
**ステータス**: ✅ Function Appのデプロイ完了

---

## ✅ 完了したこと

1. **Function Appの再デプロイ**
   - 改善版の`auth-verify-token`をデプロイ
   - ログ出力を追加（`[AuthVerifyToken]`）
   - エラーハンドリングを改善
   - 環境変数チェックを強化

2. **デプロイ成功**
   - Remote build succeeded!
   - すべての関数が正常に登録

---

## 🎯 次のステップ

### ステップ1: Function Appを再起動（完了したらステップ2へ）

```bash
az functionapp restart --name func-karte-ai-1763705952 --resource-group rg-karte-ai
```

### ステップ2: 数分待つ

Function Appの再起動には**1-2分**かかります。

### ステップ3: 新しいMagic Linkをリクエスト

1. **LPフォーム**にアクセス: https://stkarteai1763705952.z11.web.core.windows.net/
2. フォームに入力して送信
3. メールを受信（15分以内に確認）

### ステップ4: Magic Linkをクリック

1. メール内のリンクをクリック
2. 以下のいずれかが表示されるはず：
   - ✅ **成功**: Stripe Checkoutページに自動リダイレクト
   - ⚠️ **エラー**: 改善されたエラーページ（詳細なエラーメッセージ付き）

### ステップ5: Application Insightsでログを確認

1. Azure Portal → Function App → 監視 → ログ
2. 以下のクエリを実行：

```kusto
traces
| where timestamp > ago(30m)
| where message contains "AuthVerifyToken"
| order by timestamp desc
| take 50
```

**期待されるログ**:
```
[AuthVerifyToken] Configuration check: { hasStripe: true, ... }
[AuthVerifyToken] Creating Stripe Checkout session: { email: "...", ... }
[AuthVerifyToken] Stripe Checkout session created successfully: { sessionId: "...", ... }
```

---

## 🔍 トラブルシューティング

### 問題1: まだ401エラーが出る

**確認事項**:
1. Function Appが再起動されているか確認
2. 新しいMagic Linkを使っているか確認（古いトークンは無効）
3. Application Insightsでログを確認

### 問題2: ログが出力されない

**対処法**:
1. 数分待ってから再度確認
2. 時間範囲を広げる（`ago(1h)`）

### 問題3: エラーページが表示される

**確認事項**:
1. エラーメッセージを確認
2. Application Insightsでエラーログを確認
3. 環境変数が正しく設定されているか確認

---

## 📝 チェックリスト

- [x] Function Appの再デプロイ完了
- [ ] Function Appの再起動完了
- [ ] 新しいMagic Linkをリクエスト
- [ ] Magic Linkをクリックして動作確認
- [ ] Application Insightsでログを確認

---

## 🎯 期待される動作

1. **Magic Linkをクリック**
   ```
   https://func-karte-ai-1763705952.azurewebsites.net/api/auth-verify-token?token=xxx
   ```

2. **トークン検証成功**
   - JWTトークンを検証
   - メールアドレスを取得
   - セッショントークンを生成

3. **Stripe Checkoutセッション作成**
   - Stripe APIを呼び出し
   - Checkoutセッションを作成

4. **リダイレクト**
   - HTTP 302レスポンス
   - `Location: https://checkout.stripe.com/c/pay/cs_test_xxx`

5. **Stripe Checkout表示**
   - ユーザーが決済情報を入力

---

**作成者**: AI Assistant  
**最終更新**: 2025-11-30


